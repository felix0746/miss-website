      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4">
        <div className="flex justify-between items-center py-1.5 sm:py-2">
          ...
          <Link href="/" className="flex items-center group">
            <div className="w-14 h-12 sm:w-18 sm:h-14 relative transform transition-all duration-300 group-hover:scale-110 group-hover:rotate-1">
              <Image
            ...
            <nav className="flex flex-col space-y-0.5">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-gray-700 hover:text-red-600 transition-colors duration-200 font-medium px-3 py-1.5 rounded-md hover:bg-gray-50 text-xs sm:text-sm"
              ...
              <div className="px-3 pt-0.5">
                <Link
                  href="/contact"
                  className="block w-full bg-red-600 hover:bg-red-700 text-white px-4 py-1.5 rounded-md transition-colors duration-200 font-medium text-center text-xs sm:text-sm"
                  onClick={() => setIsMenuOpen(false)}
                >
                  立即諮詢
                </Link>
